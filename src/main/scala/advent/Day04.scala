...
object Day04 extends App {
  ...
  def isValid(passphrase: Array[String]): Boolean = {
    !passphrase.groupBy(identity).values
      .exists(repetitions => repetitions.length > 1)
  }

  def solve(x: String): Int = {
    x.split("\n")
      .map(line => line.split(" "))
      .count(passphrase => isValid(passphrase))
  }
  ...
  val input =
    """kvvfl kvvfl olud wjqsqa olud frc
      ...
      |rvbu czwpdit vmlihg spz lfaxxev zslfuto oog dvoksub""".stripMargin
